...
import nehlaCommentator from "@/assets/mascot/nehla-commentator.png";
import { StadiumBackdrop } from "@/components/atmosphere/StadiumBackdrop";
...
const Welcome = () => {
  ...
  return (
    <div className="relative flex min-h-svh flex-col overflow-hidden bg-stadium px-5 pt-6 pb-8">
      ...
      <div
        className="pointer-events-none absolute inset-0 -z-10 bg-scanlines opacity-[0.035]"
        aria-hidden="true"
      />
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center">
        ...
        >
          {/* Stadium rim light — warm amber from behind */}
          <div className="pointer-events-none absolute inset-0 -z-10 scale-110 rounded-full bg-accent/20 blur-[60px]" />
          {/* Floodlight pool — red wash from below */}
          <div className="pointer-events-none absolute inset-x-0 -bottom-4 -z-10 mx-auto h-24 w-3/4 rounded-full bg-primary/25 blur-[48px]" />

          <motion.img
            src={nehlaCommentator}
            alt="Nehla — commentatrice football"
            width={260}
            height={260}
            animate={{ y: [0, -4, 0] }}
            transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut" }}
            className="pointer-events-none select-none drop-shadow-[0_16px_28px_rgba(0,0,0,0.55)]"
            draggable={false}
          />
        </motion.div>
